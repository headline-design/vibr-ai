...
const components = {
  ...
  img: ({ className, alt, ...props }: React.ImgHTMLAttributes<HTMLImageElement>) => (
    <Image
      src={(props.src as string) || "/placeholder.svg"}
      alt={alt || ""}
      className={cn("rounded-md border", className)}
      width={typeof props.width === "string" ? Number.parseInt(props.width, 10) : (props.width as any)}
      height={typeof props.height === "string" ? Number.parseInt(props.height, 10) : (props.height as any)}
      {...props}
    />
  ),
