...
export const hashToHueColor = (hash: number, offset: number): string => {
...
export const generateSvgAvatar = (id: string, size: number): string => {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${size}px" height="${size}px" viewBox="0 0 ${size} ${size}" version="1.1">
    ...
    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
      <rect fill="url(#${gradientId})" x="0" y="0" width="${size}" height="${size}" rx="${size / 5}"/>
    </g>
  </svg>`
}
